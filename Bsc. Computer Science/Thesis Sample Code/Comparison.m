% Comparison of Simple Kelly and Markov Betting
...
cash(a) = log(new_payroll);
cash_2(a) = log(new_payroll_2);
...
% Race Simulation + Payout
i = sum(rand >= cumsum(B(1,1:num)));
new_payroll = new_payroll + ( money_down(i+1) * B(2,i+1) );

starting_payroll = new_payroll;
cash_3(a) = log(new_payroll);
...
for j = 1 : num
    if bets(j) > 0
        money_down(i) = betting_payroll * bets(j) / 4;
        new_payroll = new_payroll - money_down(i);
    else money_down(i) = 0;    
    end
    i = i + 1;
...
x = 1 : 10000;
figure;
loglog(x, cash(x), 'r', x, cash_2(x), 'g', x, cash_3(x), 'b', x, cash_4(x), 'm', 'LineWidth', 2);
ylabel('Log(Wealth)')
xlabel('Races Run')
legend('Full Kelly w/ Markov', 'Quarter Kelly w/ Markov', 'Full Kelly', 'Quarter Kelly', 'Location', 'northwest' );
